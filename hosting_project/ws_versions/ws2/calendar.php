...
if($action=="savewar") {
	...
	if($messages) {
		...
		$message='new upcoming clanwar on '.date("d.m.Y", $date).':[br][br]Opponent: [flag]'.$oppcountry.'[/flag] [url='.$opphp.']'.$opponent.'[/url] [br]League: [url='.$leaguehp.']'.$league.'[/url] [br]Info: '.$warinfo.'[br][br][url=index.php?site=calendar]link to calendar[/url]';
		...
		while($ds=mysql_fetch_array($ergebnis)) {
			...
			safe_query("INSERT INTO ".PREFIX."messenger (userID, date, fromuser, touser, title, message, viewed)
		                 values('$id', '$date', '$userID', '$id', '$title', '$message', '0')"); 
