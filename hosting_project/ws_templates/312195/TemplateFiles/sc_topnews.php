...
$ergebnis=safe_query("SELECT newsID, rubric, date FROM ".PREFIX."news WHERE newsID='".$topnewsID."' AND intern<=".isclanmember($userID)." AND published='1' LIMIT 0,1");
...
if($anz) {
	...
	if(mb_strlen($headline)>27) {
			$headline=mb_substr($headline, 0, 27);
			$headline.='...';
		}
	...
	$rubricpic='<img src="images/news-rubrics/'.getrubricpic($dn['rubric']).'" alt="'.$dn['rubric'].'" width="270" height="75" />';	
