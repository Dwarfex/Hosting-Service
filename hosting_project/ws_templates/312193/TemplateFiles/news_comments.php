...
if($newsID) {
	...
	if($ds['intern'] <= isclanmember($userID)) {
		...
		if(!file_exists($rubricpic) OR getrubricpic($ds['rubric'])=='') $rubricpic = ''; else $rubricpic = '<img src="'.$rubricpic.'" border="0" alt="" />';
		...
		foreach($message_array as $val) {
			if($showlang!=$i)	$langs.=flags('<span style="padding-left:2px"><a href="index.php?site=news_comments&amp;newsID='.$ds['newsID'].'&amp;lang='.$val['lang'].'">[flag]'.$val['lang'].'[/flag]</a></span>');
			$i++;
		}
    ...
    if(isnewsadmin($userID) or (isnewswriter($userID) and $ds['poster'] == $userID)) {
		...
		eval ("\$news = \"".gettemplate("newscomments")."\";");
		echo $news;
