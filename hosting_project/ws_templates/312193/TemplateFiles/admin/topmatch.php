...
elseif(isset($_GET['delete'])) {
	...
	if($all) {
		...
		<td align="right"><input type="button" onclick="MM_goToURL(\'parent\',\'admincenter.php?site=topmatch&amp;action=settings\');return document.MM_returnValue" value="Topmatch '.$_language->module['settings'].'" /></td>
			...
			<tr> 
					<td class="title" width="15%"><b>'.$_language->module['date'].'</b></td>
					...
					<td class="title" width="15%"><b>'.$_language->module['actions'].'</b></td>
			</tr>';
			...
			echo'<tr> 
					<td class="'.$td.'" width="15%">'.$date.' - '.$time.' Uhr</td>
					<td class="'.$td.'" width="10%"><a href="http://'.getinput($ds[matchlink]).'" target="_blank">'.getinput($ds[league]).'</a></td>					
